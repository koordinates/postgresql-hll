...
echo "--- Building PostgreSQL HLL extension"

# Get version from control file
HLL_VERSION=$(grep default_version hll.control | cut -d"'" -f2)
PG_VERSION=16

# Add build number if in CI
if [ -n "${BUILDKITE_BUILD_NUMBER-}" ]; then
  DEB_VERSION="${HLL_VERSION}+kx-ci${BUILDKITE_BUILD_NUMBER}"
else
  DEB_VERSION="${HLL_VERSION}"
fi

echo "Building postgresql-${PG_VERSION}-hll version ${DEB_VERSION}"

# Annotate build with version info
if [ -n "${BUILDKITE_AGENT_ACCESS_TOKEN-}" ]; then
  buildkite-agent meta-data set hll-version "$HLL_VERSION"
  buildkite-agent meta-data set deb-version "$DEB_VERSION"
  
  echo -e ":debian: HLL Package Version: \`${DEB_VERSION}\` for PostgreSQL ${PG_VERSION}" \
      | buildkite-agent annotate --style info --context deb-version
fi

BUILD_CONTAINER="build-hll-${BUILDKITE_JOB_ID:-local}"
STAGING_DIR="staging-hll"
...
time docker run \
  ...
  fpm -s dir -t deb \
    -n "postgresql-${PG_VERSION}-hll" \
    -v "${DEB_VERSION}" \
    -C "${STAGING_DIR}" \
    -p "build-jammy/postgresql-${PG_VERSION}-hll_${DEB_VERSION}_${DEB_ARCH}.deb" \
